...
def run():
    with sync_playwright() as p:
        ...
        page.evaluate("""
            ...
            const stops = [
                {name: "Stop 1", lat: -27.46, lng: -58.83, time: formatTime(new Date(now.getTime() - 600000))},
                {name: "Stop 2", lat: -27.47, lng: -58.84, time: formatTime(new Date(now.getTime() + 600000))}
            ];
            ...
            // Persist driver state to not use random walk
        ...
        # Let's see what simulateDriversTick does
        page.evaluate("appState.isSimulating = false; window.appState = appState; window.updateClock && window.updateClock();")
        time.sleep(1)

        drivers = page.evaluate("window.appState.simulatedDrivers")
        print("Active drivers:", drivers)
        ...
        if len(options) > 1:
            ...
            print("Stop options:", stop_options)
            if len(stop_options) > 1:
                page.select_option("#passenger-select-stop", value=stop_options[1])
                time.sleep(1)

                html = page.evaluate("document.getElementById('passenger-eta-list').innerHTML")
                print("ETA List HTML:", html)
