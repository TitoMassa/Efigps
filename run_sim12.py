...
def run():
    with sync_playwright() as p:
        ...
        # Hook the change event listener
        page.evaluate("""
            const stopSelect = document.getElementById("passenger-select-stop");
            stopSelect.addEventListener('change', () => {
                 console.log("Stop changed directly!", stopSelect.value);
                 try {
                     updatePassengerETAs(); // oh it's not global, the listener is closed over
                 } catch(e) {}
            });
        """)

        print("Selecting stop 2")
        # trigger click to simulate real user action which will trigger event listener bounded in app.js
