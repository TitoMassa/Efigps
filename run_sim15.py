...
def run():
    with sync_playwright() as p:
        ...
        # In updatePassengerETAs, there is a setInterval that triggers it!
        page.evaluate("""
            const oldInterval = setInterval;
            window.setInterval = function(fn, ms) {
                const id = oldInterval(() => {
                    try {
                        const pre = document.getElementById("passenger-eta-list").innerHTML;
                        fn();
                        const post = document.getElementById("passenger-eta-list").innerHTML;
                        if(pre !== post) {
                            console.log("Interval changed ETA List HTML:", post);
                        }
                    } catch(e) {}
                }, ms);
                return id;
