...
def run():
    with sync_playwright() as p:
        ...
        page.evaluate("""
            window.debugLogs = [];
            const originalUpdatePassengerETAs = window.updatePassengerETAs;
            // oh wait, it's not on window.
        """)

        # hook DOM
        page.evaluate("""
            const origHTML = Object.getOwnPropertyDescriptor(Element.prototype, 'innerHTML').set;
            Object.defineProperty(document.getElementById("passenger-eta-list"), 'innerHTML', {
                set: function(val) {
                    console.log("SET ETA HTML:", val);
                    origHTML.call(this, val);
                }
            });
        """)
