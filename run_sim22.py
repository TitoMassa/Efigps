...
def run():
    with sync_playwright() as p:
        ...
        # We don't have RouteLogic in window, let's just log what we have
        page.evaluate("""
            window.debugETAs = function() {
                ...
                const stopsToUse = driver.trips[tripIndex].stops;
                const targetStopIndex = parseInt(stopIndexStr, 10);
                const targetStop = stopsToUse[targetStopIndex];

                console.log("DRIVER LAT:", driver.lat, "LNG:", driver.lng);
                console.log("TARGET STOP TIME:", targetStop.time);

                // Print the current passenger eta list html
                console.log("CURRENT PASSENGER ETA LIST HTML:", els.passengerEtaList.innerHTML);
            };
        """)
