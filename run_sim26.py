...
def run():
    with sync_playwright() as p:
        ...
        page.evaluate("""
            ...
            const stops = [
                {name: "Stop 1", lat: -27.46, lng: -58.83, time: formatTime(new Date(now.getTime() - 3600*1000))},
                {name: "Stop 2", lat: -27.47, lng: -58.84, time: formatTime(new Date(now.getTime() + 3600*1000))}
            ];
        ...
        page.select_option("#passenger-select-line", value="sim_1")
        time.sleep(1)
        ...
        page.select_option("#passenger-select-stop", value=stop_options[1]) # Stop 1
        time.sleep(1)

        page.select_option("#passenger-select-stop", value=stop_options[2]) # Stop 2
        time.sleep(1)

        # log out everything!
        page.evaluate("""
            console.log("FINAL HTML IS:", document.getElementById("passenger-eta-list").innerHTML);
            console.log("SIMULATED DRIVERS:", window.appState.simulatedDrivers);
            if(window.appState.simulatedDrivers.length > 0) {
                console.log("DRIVER LAT LNG:", window.appState.simulatedDrivers[0].lat, window.appState.simulatedDrivers[0].lng);
                console.log("DRIVER TRIPS:", window.appState.simulatedDrivers[0].trips[0].stops);
            }
        """)
