...
def run():
    with sync_playwright() as p:
        ...
        # Add a global script into app.js execution context via mutation observer or just overwrite the innerHTML logging
        page.evaluate("""
            const target = document.getElementById('passenger-eta-list');
            const observer = new MutationObserver((mutations) => {
                mutations.forEach((mutation) => {
                    console.log("ETA LIST MUTATION:", target.innerHTML);
                });
            });
