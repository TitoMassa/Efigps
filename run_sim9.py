...
def run():
    with sync_playwright() as p:
        ...
        # In app.js, what does RouteLogic.calculateDeviation return when we are "in wait"?
        page.evaluate("""
            window.interceptDOM = () => {
                ...
                Object.defineProperty(els, 'innerHTML', {
                    set: function(val) {
                        if(val === '') {
                             // console.trace("Set to empty string");
                        }
            ...
            // Intercept app.js function to figure out where it returns
            const updateFn = window.appState; // we have window.appState
        """)
        ...
        page.evaluate("""
            const st = document.getElementById("passenger-select-stop").value;
            const ln = document.getElementById("passenger-select-line").value;
            const rt = document.getElementById("passenger-select-route").value;
            console.log("Stop:", st, "Line:", ln, "Route:", rt);
        """)
